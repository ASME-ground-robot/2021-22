...
class ScorbotAction2(object):
	...
	self._alive = True
        ...
        self.jointCmd_pub= rospy.Publisher('comandToSend', ScorbotPosition, queue_size=1)
        self.k=0
    ...
    def _send_position(self,pos):
        ...
        my_pos=[pos.positions[0],pos.positions[1],pos.positions[2]-pos.positions[1],0,0,0]
        motor=[0,1,2,3,4,5]
        i=0
        for ptn in my_pos:
            comandsToSend.ScorbotAction[i].header.stamp = rospy.Time.now()
            comandsToSend.ScorbotAction[i].header.frame_id ='positions'+str(i) 
            comandsToSend.ScorbotAction[i].motor=motor[i]
            comandsToSend.ScorbotAction[i].action=1
            comandsToSend.ScorbotAction[i].position=ptn
            comandsToSend.ScorbotAction[i].velocity=200
            print(i)
            i+=1


        self.jointCmd_pub.publish(comandsToSend)
	...
	trajectory_points = goal.trajectory.points
      # Load parameters for trajectory
        ...
        self._send_position(last)
        while (now_from_start < (last_time + self._goal_time)
               and not rospy.is_shutdown()):
            ##vamos a enviar la realimentacion y a parar de emergencia en caso que se necesite
            now_from_start = rospy.get_time() - start_time
            self._update_feedback(deepcopy(last), joint_names,
                                  now_from_start)
#aqui tenemos que usar Preemted
            if self._server.is_preempt_requested():
                rospy.logwarn("{0}: Trajectory execution Preempted. Stopping execution.".format(self._action_name))
...
if __name__ == '__main__':
    ...
    server = ScorbotAction2('scorbot/joint_trajectory_action',10)
    rospy.spin()
